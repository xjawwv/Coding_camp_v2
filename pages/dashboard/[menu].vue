...
onMounted(async () => { if (!user.value && !await loadUser()) await navigateTo({ path: '/login', query: { redirect: '/dashboard/overview' } }); try { const result = await $fetch<{ courses: any[] }>('/api/courses'); if (result.courses.length) courseList.value = result.courses } catch {} try { learningProgress.value = (await $fetch<{ progress: any[] }>('/api/progress')).progress } catch {} loading.value = false })
