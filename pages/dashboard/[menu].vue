  ...
  <div v-else-if="user" class="user-dashboard-shell">
    ...
    <main class="user-dashboard-main">
      ...
      <section v-else class="user-dashboard-layout">
        <div class="user-dashboard-primary"><section class="user-dashboard-card user-dashboard-overview-card"><UIcon name="i-lucide-layout-dashboard" /><div><p class="user-dashboard-eyebrow">ACCOUNT OVERVIEW</p><h2>Selamat datang, {{ user.name }}</h2><p>Dashboard progress belum memiliki data aktivitas untuk ditampilkan.</p></div></section><section class="user-dashboard-card user-dashboard-summary-grid"><article><UIcon name="i-lucide-book-open" /><span>Course tersedia</span><strong>{{ courseList.length }}</strong></article><article><UIcon name="i-lucide-shield-check" /><span>Status akun</span><strong>Aktif</strong></article><article><UIcon name="i-lucide-badge-check" /><span>Role akun</span><strong>{{ user.role === 'admin' ? 'Admin' : 'User' }}</strong></article></section><section class="user-dashboard-card user-dashboard-empty-card"><UIcon name="i-lucide-chart-no-axes-column" /><h2>Belum ada progress belajar</h2><p>Progress, aktivitas, dan statistik akan muncul setelah kamu mulai mengikuti course.</p><NuxtLink to="/dashboard/course" class="user-dashboard-continue">Pilih course <UIcon name="i-lucide-arrow-right" /></NuxtLink></section></div>
